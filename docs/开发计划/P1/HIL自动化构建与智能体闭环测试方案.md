...
| `T-ABNF-002` | 数值不变 ErMax 过滤 | P1 | ✅ PASS | 2/2 | TCP keepalive 修复后连接稳定，300 秒测试通过 |
...
| `T-HIST-001-A` | 三组历史数据冻结 | P0 | ⚠️ 3/4 | 对时+时间匹配 PASS，跨小时等待超时（需等实际时钟过整点） |
...
| `T-HOT-006` | 温度补偿值热更新 | P0 | ✅ PASS | 3/3 | 补偿写入后 ActualTemp 正确刷新 |
| `T-HOT-007` | 湿度补偿值热更新 | P0 | ✅ PASS | 3/3 | 补偿写入后 ActualHumi 正确刷新 |
...
**总计**：P0 项 **12 项**（通过 6 项，部分通过 5 项，失败 1 项）；P1 项 **8 项**（通过 2 项，部分通过 3 项，失败 1 项，未测 2 项）
