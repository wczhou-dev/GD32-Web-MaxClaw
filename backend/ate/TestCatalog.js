    ...
    this._businessItems = [
      ...
      // 加热控制测试子项 (ID 105-108)
      {
       ...
       判定类型: 'PASS/FAIL',
        ...
        dependencies: [],
        description: '验证加热参数读写、室内加热开启/关闭、HeatRunState 状态转换、继电器输出',
        steps: [
          '读写加热参数并验证回读一致',
          '设置温度低于开启阈值，验证室内加热开启',
          '设置温度高于关闭阈值，验证室内加热关闭',
          '验证 HeatRunState 状态机转换（IDLE→ON→CLOSE_PENDING→OFF_HOLD→IDLE）',
          '验证加热继电器输出与状态一致',
        ],
      },
      {
       ...
       判定类型: 'PASS/FAIL',
        ...
        dependencies: [],
        description: '验证加热参数越界修正和阈值间隔合法性校验',
        steps: [
          '写入超出有效范围的参数，验证被修正为默认值',
          '设置 closedTemp - openTemp < 0.5，验证触发非法保护',
          '回读参数确认修正结果',
        ],
      },
      {
       ...
       判定类型: 'PASS/FAIL',
        ...
        dependencies: [],
        description: '验证 HeatRunState 与通风等级、补偿联动的交互逻辑',
        steps: [
          '设置 HeatRunState=0（IDLE），验证通风等级可正常调节',
          '设置 HeatRunState=1（加热中），验证通风等级不可上升',
          '验证 HeatRunState!=0 时气体/湿度补偿被屏蔽',
        ],
      },
      {
        id: 108,
       ...
       判定类型: 'PASS/FAIL',
        ...
        dependencies: [],
        description: '验证温度曲线自动/手动模式切换、曲线参数读写和 Expected_temp 计算',
        steps: [
          '切换温度控制模式（自动↔手动），验证模式生效',
          '读写曲线参数（段数、日龄、温度），验证回读一致',
          '设置猪只日龄，验证 Expected_temp 按曲线正确计算',
        ],
      },
